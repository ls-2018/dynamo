...
#delete ns lws-system kai-scheduler grove-system dynamo-system kai-resource-reservation
...
test -e ./dynamo-crds-${RELEASE_VERSION}.tgz || {
...
# 3. Install Platform
test -e ./dynamo-platform-${RELEASE_VERSION}.tgz || {
  helm fetch https://helm.ngc.nvidia.com/nvidia/ai-dynamo/charts/dynamo-platform-${RELEASE_VERSION}.tgz
}

#tar -zxvf dynamo-platform-${RELEASE_VERSION}.tgz
#cd dynamo-platform

helm template dynamo-platform dynamo-platform-${RELEASE_VERSION}.tgz \
  --namespace ${NAMESPACE} \
  --create-namespace \
  --set etcd.persistence.enabled=false \
  --set dynamo-operator.controllerManager.manager.image.pullPolicy=Always \
  --set nats.natsBox.enabled=true \
  --set nats.config.jetstream.fileStore.pvc.enabled=false \
  --set dynamo-operator.dynamo.internalImages.debugger=ccr.ccs.tencentyun.com/acejilam/ib-lh764rgbdj:74d7f6afef5276ca130305846c5b0675-3.12-slim \
  --dry-run >./dynamo.yaml
...
kubectl -n dynamo-system patch deployment dynamo-platform-dynamo-operator-controller-manager \
  ...
  -p='[
          ...
          },
          {
              ...
              "value": "registry.cn-hangzhou.aliyuncs.com/ls-2018/test:dyt"
          }
      ]'
...
#.
## Test it
kubectl port-forward svc/vllm-agg-frontend 8000:8000 -n dynamo-system
